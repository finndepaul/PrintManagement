...
namespace PrintManagement.Application.ImplementServices
{
	public class AuthService : IAuthService
	{
		...
		public AuthService(UserConverter userConverter, IBaseRepository<User> baseUserRepository, IBaseRepository<ConfirmEmail> baseConfirmEmailRepository, IUserRepository userRepository, IEmailService emailService)
		{
		...
		public async Task<ResponseObject<DataResponseUser>> Register(RegisterRequest request, CancellationToken cancellationToken)
		{
			try
			{
				...
				if (await _userRepository.GetTeamIdAsync(request.TeamName, cancellationToken) == null)
				{
					return new ResponseObject<DataResponseUser>
					{
						Status = StatusCodes.Status400BadRequest,
						Message = "Team name không tồn tại! Vui lòng sử dụng username khác",
						Data = null
					};
				}
				var user = new User
				{
					...
					TeamId = await _userRepository.GetTeamIdAsync(request.TeamName, cancellationToken),
